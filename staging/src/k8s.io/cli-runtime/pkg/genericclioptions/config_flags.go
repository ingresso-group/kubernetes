...
import (
	...
	rediscached "k8s.io/client-go/discovery/cached/redis"
...
func (f *ConfigFlags) toDiscoveryClient() (discovery.CachedDiscoveryInterface, error) {
	...
	httpCacheDir = filepath.Join(cacheDir, "http")
	discoveryCachePrefix := computeDiscoverCachePrefix(config.Host)
	return rediscached.NewCachedDiscoveryClientForConfig(config, discoveryCachePrefix, httpCacheDir, time.Duration(10*time.Minute))
}
...
func computeDiscoverCachePrefix(host string) string {
	// strip the optional scheme from host if its there:
	schemelessHost := strings.Replace(strings.Replace(host, "https://", "", 1), "http://", "", 1)
	// now do a simple collapse of non-AZ09 characters.  Collisions are possible but unlikely.  Even if we do collide the problem is short lived
	safeHost := overlyCautiousIllegalFileCharacters.ReplaceAllString(schemelessHost, "_")
